...
def get_args():
    ...
    parser.add_argument('--train-dir', required=True, help='path to folder with pre-processed files (.npz)')
    ...
    parser.add_argument('--checkpoint', type=str, default='/media/aranem_locale/AR_subs_exps/SAM_white/checkpoints/sam_vit_l_0b3195.pth', help='checkpoint')
...
def _train(args):
    ...
    train_lists = [list(v) for _, v in itertools.groupby(train_files, key=lambda x:x.split(os.sep)[8])]  # --> task names and IDs
    ...
    generator = npz_generator(train_files, nr_samples=nr_samples, jitter=args.jitter, use_only_centroid_of_gt=args.use_only_centroid_of_gt, use_only_center_of_bbox=args.use_only_center_of_bbox, use_quarter_four_points=args.use_quarter_four_points, batch_size=args.batch_size)
    ...
    if args.load_model:
        ...
        model = SAM(inshape=inshape, nr_samples=nr_samples, neg_samples=args.neg_samples, bbox=args.use_bbox, model_type=args.model_type, checkpoint=args.checkpoint, device=device)
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        if epoch % args.save_steps == 0:
            ...
            validate(model, val_files, epoch, store_samples=True, out_=os.path.join(args.model_dir, "validation"), use_neg_samples=args.neg_samples, use_bbox=args.use_bbox, jitter=args.jitter, use_only_centroid_of_gt=args.use_only_centroid_of_gt, use_only_center_of_bbox=args.use_only_center_of_bbox, use_quarter_four_points=args.use_quarter_four_points)
        ...
        for _ in range(args.steps_per_epoch):
            ...
            imgs, segs, embeds, orig_sizes, input_sizes, bboxs_gt_, samples_gt_, neg_samples_gt_ = next(generator)
            ...
            y_pred_ = model(imgs, embeds, orig_sizes, input_sizes, samples_gt_, neg_samples_gt_, bboxs_gt_, train=True, freeze_sam_body=args.freeze_sam_body, freeze_sam_head=args.freeze_sam_head)
    ...
    validate(model, val_files, args.epochs, store_samples=True, out_=os.path.join(args.model_dir, "validation"), use_neg_samples=args.neg_samples, use_bbox=args.use_bbox, jitter=args.jitter, use_only_centroid_of_gt=args.use_only_centroid_of_gt, use_only_center_of_bbox=args.use_only_center_of_bbox, use_quarter_four_points=args.use_quarter_four_points)
    print("")
