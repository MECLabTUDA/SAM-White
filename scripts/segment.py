...
def get_args():
    ...
    parser.add_argument(
        "--npz",
        required=True,
        help="pre-processed img npz file with embeddings and other stuff",
    )
    parser.add_argument(
        "--model", required=True, help="pytorch model for nonlinear registration"
    )
    parser.add_argument(
        "-g", "--gpu", help="GPU number(s) - if not supplied, CPU is used"
    )
    parser.add_argument(
        "--store_npz",
        action="store_true",
        help="Set this if the input should be copied as well --> takes a lot of space..",
    )
    parser.add_argument(
        "--use_bbox",
        action="store_true",
...
def _segment(args):
    ...
    imgs, segs, _, _, _, _, _, _ = d_utils.load_npzfile(
        args.npz,
        model.nr_samples,
        jitter=args.jitter,
    ...
    model_dir = os.path.join(*args.model.split(os.sep)[:-1])

    # Do validation for one sample based on args.npz
    _, val_res, y_pred_ = validate(
        model,
        [args.npz],
        epoch,
        store_samples=True,
        out_=os.path.join(model_dir, "inference"),
    ...
    # Store npz input file
    if args.store_npz:
        shutil.copy(args.npz, os.path.join(args.out, "input.npz"))

    # Store image and GT segmentation
    sitk.WriteImage(
        sitk.GetImageFromArray(imgs[..., 0]), os.path.join(args.out, "img.nii.gz")
    )
    sitk.WriteImage(
        sitk.GetImageFromArray(segs.squeeze()), os.path.join(args.out, "seg_gt.nii.gz")
    )
    sitk.WriteImage(
        sitk.GetImageFromArray(segs.squeeze() * 255), os.path.join(args.out, "seg_gt.png")
    )

    # Store predicted segmentation
    sitk.WriteImage(
        sitk.GetImageFromArray(y_pred_), os.path.join(args.out, "pred_seg.nii.gz")
    )
    sitk.WriteImage(
        sitk.GetImageFromArray(y_pred_ * 255), os.path.join(args.out, "pred_seg.png")
    )
