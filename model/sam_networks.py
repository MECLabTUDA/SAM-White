...
class SAM(LoadableModel):
    ...
    """
    @store_config_args
    def __init__(self, inshape, nr_samples, neg_samples=False, bbox=False, model_type="vit_b", checkpoint="/home/aranem_locale/Desktop/SAM_CL/checkpoints/sam_vit_b_01ec64.pth", device='cpu'):
    ...
    def forward(self, x, x_embed, orig_size, input_size, samples_c, neg_samples_c=None, bbox_c=None, train=True, freeze_sam_body=False, freeze_sam_head=False, **kwargs):
        r"""
            Forward pass.
        ...
        """
        if self.use_bbox:
            assert bbox_c is not None, "If you want to use bbox, then provide a bbox that is not None."

        segs = list()
        for i, _ in enumerate(x):
            # -- Build prompt -- #
            samples_, neg_samples_ = copy.deepcopy(samples_c[i]), copy.deepcopy(neg_samples_c[i])   # <-- Otherwise in-place changes
            ...
            neg_samples_len = 0 if isListEmpty(neg_samples_c[i]) else len(neg_samples_c[i])

            if self.use_neg_samples:
                input_points = extract_samples_from_tensor(samples_, samples_len+neg_samples_len)   # Size([nr_samples, 2])
            else:
                input_points = extract_samples_from_tensor(samples_, samples_len)   # Size([nr_samples, 2])

            input_labels = np.array([1]*samples_len)  # Corresponding label for the points --> since binary for now, always 1
            input_labels_neg = np.array([0]*neg_samples_len)  # Corresponding label for the points --> since binary for now, always 0
            ...
            with torch.set_grad_enabled(not freeze_sam_body):    # If freeze_sam_body True then use torch.no_grad
                # -- Set the slice in SAM manually using the already extracted embedding -- #
                self.features = x_embed[i].clone()  # <-- Only if freeze_sam_body, otherwise calculate them new
                if freeze_sam_body:
                ...
                if self.use_bbox:
                    ...
                    for box in bboxs:
                        if len(box) != 0:
                            bboxs_new.append(box)   # <-- If even one box is [], then don't add it
                    if len(bboxs_new) != 0:
                        box_ = self.transform.apply_boxes(np.asarray(bboxs_new), self.original_size)
                        box_torch = torch.as_tensor(box_, dtype=torch.float, device=self.device).unsqueeze(1)
                    else: # <-- If all provided bboxs are [], then set bbox to None
                        box_torch = None

                # Embed prompts
                sparse_embeddings, dense_embeddings = self.sam_model.prompt_encoder(
                    points=(coords_torch, labels_torch) if not isListEmpty(input_points) else None,
                    boxes=box_torch if self.use_bbox else None,
                    masks=None,
                )

            # Predict masks
            with torch.set_grad_enabled(not freeze_sam_head):   # If freeze_sam_head True then use torch.no_grad
...
class SAM_UNet(SAM):
    @store_config_args
    def __init__(self, inshape, nr_samples, neg_samples=False, bbox=False, model_type="vit_b", checkpoint="/home/aranem_locale/Desktop/SAM_CL/checkpoints/sam_vit_b_01ec64.pth", device='cpu'):
        super(SAM_UNet, self).__init__(inshape, nr_samples, neg_samples, bbox, model_type, checkpoint, device)
        self.unet = UNet()

    def forward(self, x, x_embed, orig_size, input_size, samples_c, neg_samples_c=None, bbox_c=None, train=True, freeze_sam_body=False, freeze_sam_head=False, freeze_unet=False, **kwargs):
        r"""
            Forward pass.
        """
        ...
        segs = super(SAM_UNet, self).forward(x, x_embed, orig_size, input_size, samples_c, neg_samples_c, bbox_c, True, False, freeze_sam_head, **kwargs)
        # Forward UNet
        if not freeze_unet:
            segs = self.unet(segs)
...
class UNet(LoadableModel):
    ...
    def forward(self, x, frozen=False):
        with torch.set_grad_enabled(not frozen):   # If frozen True then use torch.no_grad
            # @Niklas: TODO add the forward pass here
            # forward x=UNet(x)
