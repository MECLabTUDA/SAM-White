...
def preprocess_rgb(gt_path, nii_path, gt_name, image_name, label_id, image_size, sam_model, device):
    r"""
        This extracts the mbeddings and stores them, it does not create 3 channels like for MRI/CT.
    """
...
for task in tasks:
    for model_type, checkpoint in zip(model_types,checkpoints):
        ...
        for name in tqdm(names):
            ...
            gt_name = name
            orig, imgs, gts, img_embeddings, original_size, input_size = preprocess_rgb(args.gt_path, args.nii_path, gt_name, image_name, args.label_id, args.image_size, sam_model, args.device)
            # save to npz file
